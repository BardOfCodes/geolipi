...
from geolipi.symbolic.transforms_2d import Translate2D, EulerRotate2D, Scale2D
...
def str_to_expr(expression_string_list: List[str], to_cuda=False, dtype=th.float32):
    ...
    for expression_string in expression_string_list[::-1]:
        ...
        if cmd_name in BOOL_MAP.keys():
            ...
            translate_param = params[:2]
            scale_param = params[2:4]
            rotate_param = params[4:5]
            cmd = Translate2D(EulerRotate2D(
                    Scale2D(PRIM_MAP[cmd_name](), scale_param), 
