...
def helper_polyline_sdf_arc(points, sc, ra):
    points[..., 0] = th.abs(points[..., 0])
    dist_1 = th.norm(points - sc * ra, dim=-1)
    dist_2 = th.abs(th.norm(points, dim=-1) - ra)
    sdf = th.where(sc[0, 1] * points[..., 0] > sc[0, 0] * points[..., 1], dist_1, dist_2)
    return sdf

def helper_polyline_sc_from_ht(tan_half_x):
    tan_half_sq = tan_half_x * tan_half_x
    denom = 1.0 + tan_half_sq
    sc = th.stack([(2.0 * tan_half_x) / denom, (1.0 - tan_half_sq) / denom], -1)
    return sc
...
def helper_polyline_bulge_arc(points, a, b, bulge):
    ...
    orth_ba = ortho_ba / (l + EPSILON)
    tan_tby2 = 2.0 * bulge / (1.0 - bulge * bulge)
    h = l / (2.0 * tan_tby2)
    ...
    rot_points = th.bmm(rot_mat, relative_p.unsqueeze(1)).squeeze(1)
    ...
    min_y = th.amin(a[..., 1], b[..., 1])
    max_y = th.amax(a[..., 1], b[..., 1])
    in_range =  min_y < points[..., 1] < max_y
    ...
    arc_max_y = th.amax(consider)
    arc_min_y = -th.amax(-consider)
