...
def sdf2d_rounded_box(points, bounds, radius):
    # points shape [batch, num_points, 2]
    # bounds shape [batch, 2]
    # radius shape [batch, 4]
...
def sdf2d_oriented_box(points, start_point, end_point, thickness):
    # points shape [batch, num_points, 2]
    # start_point shape [batch, 2]
    # end_point shape [batch, 2]
    # thickness shape [batch, 1]
    ...
    q = q - lt
    sdf = th.norm(th.clamp(q, min=0.0), dim=-1) + \
    th.clamp(th.amax(q, dim=-1), max=0.0)
    return sdf
...
def sdf2d_equilateral_triangle(points, side_length):
    ...
    points[..., 0] = points[..., 0] - th.clamp(th.clamp(points[..., 0].clone(), min=-2 * side_length), max=0)
...
def sdf2d_regular_pentagon(points, r):
    ...
    k2= k.clone()
    k2[0] *=-1
    points = points - 2 * (th.clamp((points * k2[:2]).sum(-1), max=0.)[..., None] * k2[..., :2])
    points = points - 2 * (th.clamp((points * k[:2]).sum(-1), max=0.)[..., None] * k[..., :2])
    points[..., 0] = points[..., 0] - th.clamp(points[..., 0].clone(), -r * k[2], r * k[2])
    points[..., 1] = points[..., 1] - r
    sdf = th.norm(points, dim=-1) * th.sign(points[..., 1])
    return sdf
...
def sdf2d_hexagram(points, r):
    ...
    points = points - 2 * th.clamp((points * k[:2]).sum(-1), max=0.)[..., None] * k[..., :2]
    points = points - 2 * th.clamp((points * k2[2:]).sum(-1), max=0.)[..., None] * k2[..., 2:]
    points[..., 0] = points[..., 0] - th.clamp(points[..., 0].clone(), r * k[2], r * k[3])
    points[..., 1] = points[..., 1] - r
    sdf = th.norm(points, dim=-1) * th.sign(points[..., 1])
    return sdf
...
def sdf2d_star_5(points, r, rf):
    ...
    k2[0] = k2[0] * -1
    points[..., 0] = th.abs(points[..., 0])
    points = points - 2 * th.clamp((points * k).sum(-1), min=0.)[..., None] * k[..., :]
    points = points - 2 * th.clamp((points * k2).sum(-1), min=0.)[..., None] * k2[..., :]
    points[..., 0] = th.abs(points[..., 0])
    points[..., 1] = points[..., 1] - r
    ba = k2.clone() * -1 
...
def sdf2d_pie(points, c, r):
    ...
    m = th.norm(points - c * th.clamp(th.clamp((points * c).sum(-1), min=0), max=r)[..., None], dim=-1)
...
def sdf2d_arc(points, angle, ra, rb):
    ...
    sdf = th.where(pointer, th.norm(points - sc[..., None, :] * ra[..., None, :], dim=-1),
...
def sdf2d_horse_shoe(points, angle, r, w):
    ...
    out = points.clone()
    m = points[..., 0] > 0
    out[..., 0] = th.where((points[..., 1] > 0) | m, points[..., 0], l * th.sign(-c[..., 0:1]))
    out[..., 1] = th.where(m, points[..., 1], l)
    out[..., 0] = out[..., 0] - w[..., 0:1]
    out[..., 1] = th.abs(out[..., 1] -r) - w[..., 1:2]
    sdf = th.norm(th.clamp(out, min=0.0), dim=-1) + th.clamp(th.amax(out, dim=-1), max=0.0)
    return sdf
...
def sdf2d_vesica(points, r, d):
    ...
    p1[..., 1] = p1[..., 1] - b
    p2[..., 0] = p2[..., 0] + d
    sdf = th.where((points[..., 1] - b) * d > points[..., 0] * b, 
                   th.norm(p1, dim=-1),
                   th.norm(p2, dim=-1) - r)
    return sdf
...
def sdf2d_rounded_cross(points, h):
    ...
    sdf = th.where(cond, k - all_norm[..., 0], th.minimum(all_norm[..., 1], all_norm[..., 2]))
    return sdf
...
def sdf2d_egg(points, ra, rb):
    ...
    sdf = th.where(cond_1, all_norm[..., 0] - r,
                   th.where(cond_2, all_norm[..., 1],
                            all_norm[..., 2] - 2.0 * r ))
    sdf = sdf - rb
    return sdf
...
def sdf2d_heart(points):
    ...
    all_p = th.stack([points, p2, p3], -1)
    all_norm = th.norm(all_p, dim=-2)
    sdf = th.where(cond, all_norm[..., 0] - SQRT_2/4.0,
...
def sdf2d_polygon(points, verts):
    # points shape [batch, num_points, 2]
    # verts shape [batch, num_vertices, 2]
    if len(points.shape) == 2:
    #     points = points.unsqueeze(0)
...
def sdf2d_parabola(points, k):
    ...
    sol_1 = th.pow(th.abs(q + r) + EPSILON, 1.0/3.0) - th.pow(th.abs(q - r) + EPSILON, 1.0/3.0) * th.sign(r - q)
...
def sdf2d_blobby_cross(points, he):
    ...
    pos = th.stack([th.abs(points[..., 0] - points[..., 1]), 1.0 - points[..., 0] - points[..., 1]], -1) / SQRT_2
    p = (he - pos[..., 1] - 0.25/(he + EPSILON)) / (6.0 * he + EPSILON)
    q = pos[..., 0] / (he * he * 16.0 + EPSILON)
    h = q * q - p * p * p
    r_1 = th.sqrt(th.clamp(h, min=EPSILON))
    x_1 = th.pow(th.abs(q + r_1) + EPSILON, 1.0/3.0) - th.pow(th.abs(q - r_1) + EPSILON, 1.0/3.0) * th.sign(r_1 - q)
    r_2 = th.sqrt(th.clamp(p, min=EPSILON))
    acos_in = th.clamp(q / (p * r_2 + EPSILON), min=-1 + ACOS_EPSILON, max=1 - ACOS_EPSILON)
...
def sdf2d_tunnel(points, wh):
    ...
    points[..., 1] = -points[..., 1]
    q = points - wh[..., None, :]
    d1 = th.norm(th.stack([th.clamp(q[..., 0].clone(), min=0.0), q[..., 1]], -1), dim=-1)
...
def sdf2d_stairs(p, wh, n):
    ...
    p += ba[..., None, :]/2.0
    p1 = p.clone()
    p1[..., 0] = p1[..., 0] - th.clamp(th.clamp(p[..., 0], min=0.0), max=ba[..., None, 0])
    d1 = (p1 * p1).sum(-1)
    p2 = p.clone()
    p2[..., 1] = p2[..., 1] - th.clamp(th.clamp(p[..., 1], min=0.0), max=ba[..., None, 1])
    p2[..., 0] = p2[..., 0] - ba[..., None, 0]
    d2 = (p2 * p2).sum(-1)
    d = th.minimum(d1, d2)
...
def sdf2d_quadratic_circle(points):
    ...
    acos_in = th.clamp(a / (c * z2 + EPSILON), min=-1 + ACOS_EPSILON, max=1 - ACOS_EPSILON)
...
def sdf2d_circle_wave(points, tb, ra):
    ...
    points[..., 0] = th.abs(th.fmod(points[..., 0].clone(), co[..., 0:1] * 4.0) - co[..., 0:1] * 2.0)
    p1 = points.clone()
    p2 = th.stack([th.abs(points[..., 0] - 2.0 * co[..., 0:1]), -points[..., 1] + 2.0 * co[..., 1:2]], -1)
    cond = co[..., 1:2] * p1[..., 0] > co[..., 0:1] * p1[..., 1]
    d1 = th.where(cond, th.norm(p1 - co[..., None, :], dim=-1), th.abs(th.norm(p1, dim=-1) - ra))
    cond = co[..., 1:2] * p2[..., 0] > co[..., 0:1] * p2[..., 1]
    d2 = th.where(cond, th.norm(p2 - co[..., None, :], dim=-1), th.abs(th.norm(p2, dim=-1) - ra))
...
def sdf2d_hyperbola(points, k, he):
    ...
    # he shape [batch, 1]
    points = th.abs(points)
    points = th.stack([points[..., 0] - points[..., 1], points[..., 0] + points[..., 1]], -1) / SQRT_2
    ...
    acos_in = th.clamp(q / (r * m1 + EPSILON), min=-1.0 + ACOS_EPSILON, max=1.0 - ACOS_EPSILON)
    ...
    t = points[..., 0] / 4.0 - w + th.sqrt(th.clamp(2.0 * x2 - u + b / (w * 4.0 + EPSILON), min=EPSILON))
...
def sdf2d_quadratic_bezier_curve(points, A, B, C):
    ...
    ky = kk * (2.0 * (a * a).sum(-1, keepdim=True) + (d * b[..., None, :]).sum(-1)) / 3.0
    ...
    acos_in = th.clamp(q / (p * z2 * 2.0 + EPSILON), min=-1.0 + ACOS_EPSILON, max=1.0 - ACOS_EPSILON)
    v2 = th.acos(acos_in) / 3.0
    m2 = th.cos(v2)
    n2 = th.sin(v2) * SQRT_3
    t2 = th.clamp(th.stack([m2 + m2, -n2 - m2, n2 - m2], -1) * z2[..., None] - kx[..., None], min=0.0, max=1.0)
    res_2 = th.minimum(th.norm(d + (c + b * t2[..., 0:1]) * t2[..., 0:1], dim=-1),
